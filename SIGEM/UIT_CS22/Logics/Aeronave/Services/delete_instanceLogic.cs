...
namespace SIGEM.Client.Logics.Aeronave.Services
{
	...
	public static class delete_instanceLogic
	{
		...
		/// </summary>
		/// <param name="p_thisAeronaveArg">Value of the inbound argument 'p_thisAeronave'.</param>
		/// <returns>List of inbound arguments values.</returns>
		private static Dictionary<string, object> GetInputFieldValues(AeronaveOid p_thisAeronaveArg)
		{
			// Fill values dictionary.
			Dictionary<string, object> lValues = new Dictionary<string, object>();
			lValues.Add("p_thisAeronave", p_thisAeronaveArg);
		...
		public static Dictionary<string, ModelType> GetInboundArgumentTypes()
		{
			...
			lTypes.Add("p_thisAeronave", ModelType.Oid);
		...
		public static Dictionary<string, string> GetInboundArgumentDomains()
		{
			...
			lDomains.Add("p_thisAeronave", "Aeronave");
		...
		public static void ExecuteLoadFromContext(IUServiceContext context)
		{
			...
			// Argument 'this' initialization: 'p_thisAeronave'.
			...
			if (UtilFunctions.OidsBelongToClass(lSelectedOids, "Aeronave"))
			{
				if (!context.GetInputFieldMultiSelectionAllowed("p_thisAeronave") && lSelectedOids.Count > 1)
				{
					lSelectedOids.RemoveRange(1, lSelectedOids.Count - 1);
				}
				previousValue = context.GetInputFieldValue("p_thisAeronave");
				context.SetInputFieldValue("p_thisAeronave", lSelectedOids);
				// Check SetValue dependency rules.
				context.SelectedInputField = "p_thisAeronave";
				...
				context.SetInputFieldEnabled("p_thisAeronave", false);
				context.SelectedInputField = "p_thisAeronave";
			...
			if (previousValue == null)
			{
				...
				{
					context.SetInputFieldValue("p_thisAeronave", lArgumentValue);
					// Check SetValue dependency rules.
					context.SelectedInputField = "p_thisAeronave";
		...
		{
			context.OutputFields = ExecuteService(context.Agent, context.GetInputFieldValue("p_thisAeronave") as List<Oid>);
		}
		/// <summary>
		/// Method that solves the execution of 'delete_instance' service.
		/// </summary>
		...
		{
			return ExecuteService(agent, argumentValues["p_thisAeronave"] as List<Oid>);
		}
		/// <summary>
		/// Method that solves the execution of 'delete_instance' service.
		/// </summary>
		...
		/// <param name="p_thisAeronaveArg">Value of the inbound argument 'p_thisAeronave'.</param>
		/// <returns>Outbound argument values.</returns>
		public static Dictionary<string, object> ExecuteService(Oid agent, List<Oid> p_thisAeronaveArg)
		{
			AeronaveOid lp_thisAeronave = null;
			if ((p_thisAeronaveArg != null) && (p_thisAeronaveArg.Count > 0) && (p_thisAeronaveArg[0] != null))
			{
				lp_thisAeronave = (p_thisAeronaveArg[0] as AeronaveOid);
			}
			Dictionary<string, object> lValues = GetInputFieldValues(lp_thisAeronave);
			Dictionary<string, ModelType> lTypes = GetInboundArgumentTypes();
			Dictionary<string, string> lDomains = GetInboundArgumentDomains();

			Dictionary<string, object> lOutboundArguments = Logic.Adaptor.ExecuteService(agent, "Aeronave", "delete_instance", lTypes, lValues, lDomains);
		...
		public static void ExecuteValidateValue(IUServiceContext context)
		{
			...
			{	
				case "p_thisAeronave":
					ExecuteValidateValuep_thisAeronave(context.Agent, context.GetInputFieldValue("p_thisAeronave") as List<Oid>);
					break;
				default:
					break;
			}
		...
		public static void ExecuteValidateValue(Oid agent, string argumentName, object argumentValue)
		{
			...
			{	
				case "p_thisAeronave":
					ExecuteValidateValuep_thisAeronave(agent, argumentValue as List<Oid>);
					break;
				default:
					break;
			}
		...
		/// <summary>
		/// Solves the validation of the 'p_thisAeronave' inbound argument.
		/// </summary>
		/// <param name="agent">Application agent</param>
		/// <param name="argumentValue">Value of the inbound argument to validate</param>
		public static void ExecuteValidateValuep_thisAeronave(Oid agent, List<Oid> argumentValue)
		{
			// TODO: Validate value
		}
