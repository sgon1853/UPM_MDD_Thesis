...
namespace SIGEM.Client.Logics.Pasajero
{
	/// <summary>
	...
	public static class ClassLogic
	{
		...
		public static DataTable ExecuteQueryInstance(Oid agent, Oid oid, string displaySet)
		{
			try
			{
				return ExecuteQueryInstance(agent, new PasajeroOid(oid), displaySet);
			}
			catch
			{
				return null;
		...
		public static DataTable ExecuteQueryRelatedPasajeroAeronave(Oid agent, PasajeroAeronaveOid relatedOid, string displaySet, string orderCriteria, PasajeroOid lastOid, int blockSize, ref bool lastBlock)
		{
			Dictionary<string, Oid> lLinkItems = new Dictionary<string, Oid>(StringComparer.CurrentCultureIgnoreCase);
		...
		/// <summary>
		/// Decides if the action SIU_create_instance of Pasajero has to be enabled or disabled ("1" / "0").
		/// </summary>
		...
		private static string Action_Pasajero_SIU_create_instance(IUQueryContext context, DataRow row, string actionItemIdXML)
		{
			return "1";
		}

		/// <summary>
		/// Decides if the action SIU_delete_instance of Pasajero has to be enabled or disabled ("1" / "0").
		/// </summary>
		/// <param name="context">IUQueryContext object received.</param>
		/// <param name="row">DataRow that contains the data of the instance selected.</param>
		/// <param name="actionItemIdXML">XML identificator of the action element.</param>
		/// <returns>A string indicating if the action element has to be enabled "1" or disabled "0".</returns>
		private static string Action_Pasajero_SIU_delete_instance(IUQueryContext context, DataRow row, string actionItemIdXML)
		{
			return "1";
		}
		...
		private static string GetEnabledNavigation(IUQueryContext context, DataRow row, string navigationKey)
		{
			try
			{
				...
				{
					case "Clas_1348178542592658NavOfer_AutoElemNav_1_Alias":
						{
		...
		public static List<string> GetIdentificationFunctionFields()
		{
			...
			lFields.Add("id_Pasajero"); //Attribute Name is id_Pasajero
