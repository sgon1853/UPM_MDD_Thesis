...
namespace SIGEM.Client.Logics.Revision.Services
{
	...
	public static class delete_instanceLogic
	{
		...
		/// </summary>
		/// <param name="p_thisRevisarAeronaveArg">Value of the inbound argument 'p_thisRevisarAeronave'.</param>
		/// <returns>List of inbound arguments values.</returns>
		private static Dictionary<string, object> GetInputFieldValues(RevisionOid p_thisRevisarAeronaveArg)
		{
			// Fill values dictionary.
			Dictionary<string, object> lValues = new Dictionary<string, object>();
			lValues.Add("p_thisRevisarAeronave", p_thisRevisarAeronaveArg);
		...
		public static Dictionary<string, ModelType> GetInboundArgumentTypes()
		{
			...
			lTypes.Add("p_thisRevisarAeronave", ModelType.Oid);
		...
		public static Dictionary<string, string> GetInboundArgumentDomains()
		{
			...
			lDomains.Add("p_thisRevisarAeronave", "Revision");
		...
		public static void ExecuteLoadFromContext(IUServiceContext context)
		{
			...
			// Argument 'this' initialization: 'p_thisRevisarAeronave'.
			...
			if (UtilFunctions.OidsBelongToClass(lSelectedOids, "Revision"))
			{
				if (!context.GetInputFieldMultiSelectionAllowed("p_thisRevisarAeronave") && lSelectedOids.Count > 1)
				{
					lSelectedOids.RemoveRange(1, lSelectedOids.Count - 1);
				}
				previousValue = context.GetInputFieldValue("p_thisRevisarAeronave");
				context.SetInputFieldValue("p_thisRevisarAeronave", lSelectedOids);
				// Check SetValue dependency rules.
				context.SelectedInputField = "p_thisRevisarAeronave";
				...
				context.SetInputFieldEnabled("p_thisRevisarAeronave", false);
				context.SelectedInputField = "p_thisRevisarAeronave";
			...
			// 'p_thisRevisarAeronave' argument.
			previousValue = context.GetInputFieldValue("p_thisRevisarAeronave");
			if (previousValue == null)
			{
				...
				{
					context.SetInputFieldValue("p_thisRevisarAeronave", lArgumentValue);
					// Check SetValue dependency rules.
					context.SelectedInputField = "p_thisRevisarAeronave";
		...
		{
			context.OutputFields = ExecuteService(context.Agent, context.GetInputFieldValue("p_thisRevisarAeronave") as List<Oid>);
		}
		/// <summary>
		/// Method that solves the execution of 'delete_instance' service.
		/// </summary>
		...
		/// <param name="p_thisRevisarAeronaveArg">Value of the inbound argument 'p_thisRevisarAeronave'.</param>
		/// <returns>Outbound argument values.</returns>
		public static Dictionary<string, object> ExecuteService(Oid agent, List<Oid> p_thisRevisarAeronaveArg)
		{
			RevisionOid lp_thisRevisarAeronave = null;
			if ((p_thisRevisarAeronaveArg != null) && (p_thisRevisarAeronaveArg.Count > 0) && (p_thisRevisarAeronaveArg[0] != null))
			{
				lp_thisRevisarAeronave = (p_thisRevisarAeronaveArg[0] as RevisionOid);
			}
			Dictionary<string, object> lValues = GetInputFieldValues(lp_thisRevisarAeronave);
			Dictionary<string, ModelType> lTypes = GetInboundArgumentTypes();
			Dictionary<string, string> lDomains = GetInboundArgumentDomains();

			Dictionary<string, object> lOutboundArguments = Logic.Adaptor.ExecuteService(agent, "Revision", "delete_instance", lTypes, lValues, lDomains);
		...
		public static void ExecuteValidateValue(IUServiceContext context)
		{
			switch (context.SelectedInputField)
			{	
				case "p_thisRevisarAeronave":
					ExecuteValidateValuep_thisRevisarAeronave(context.Agent, context.GetInputFieldValue("p_thisRevisarAeronave") as List<Oid>);
					break;
				default:
					break;
			}
		...
		public static void ExecuteValidateValue(Oid agent, string argumentName, object argumentValue)
		{
			...
			{	
				case "p_thisRevisarAeronave":
					ExecuteValidateValuep_thisRevisarAeronave(agent, argumentValue as List<Oid>);
					break;
				default:
					break;
			}
