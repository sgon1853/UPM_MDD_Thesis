...
namespace SIGEM.Client.InteractionToolkit.RevisionPasajero.IUServices
{
	...
	public partial class SIU_delete_instanceInboundIT : Form
	{
		...
		public SIU_delete_instanceInboundIT()
		{
			...
			// Icon assignament.
			this.Icon = UtilFunctions.BitmapToIcon(Properties.Resources.RevisionPasajero_SIU_SIU_delete_instance);
		}
		...
		{
			Controller = ControllerFactory.RevisionPasajero.Service_delete_instanceInbound(exchangeInfo, null);

			// Scenario.
			Controller.Scenario = new ScenarioPresentation(this, ScenarioType.Service);
			ArgumentOVController lArgumentObjectValued = null;

			// p_thisRevisionPasajero
			lArgumentObjectValued = Controller.InputFields["p_thisRevisionPasajero"] as ArgumentOVController;
			lArgumentObjectValued.Label = new LabelPresentation(this.lp_thisRevisionPasajero);
			lArgumentObjectValued.Editors[0] = new MaskedTextBoxPresentation(this.maskedTextBoxp_thisRevisionPasajeroid_RevisionPasajero1);
			lArgumentObjectValued.Trigger = new ButtonPresentation(this.bp_thisRevisionPasajero);
			if (lArgumentObjectValued.SupplementaryInfo != null)
			{
				lArgumentObjectValued.SupplementaryInfo.Viewer = new LabelDisplaySetPresentation(this.lp_thisRevisionPasajeroSupInfo);
			}
