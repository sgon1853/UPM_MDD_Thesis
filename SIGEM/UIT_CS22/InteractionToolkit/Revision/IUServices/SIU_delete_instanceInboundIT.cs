...
namespace SIGEM.Client.InteractionToolkit.Revision.IUServices
{
	...
	public partial class SIU_delete_instanceInboundIT : Form
	{
		...
		public SIU_delete_instanceInboundIT()
		{
			...
			// Icon assignament.
			this.Icon = UtilFunctions.BitmapToIcon(Properties.Resources.Revision_SIU_SIU_delete_instance);
		}
		...
		{
			Controller = ControllerFactory.Revision.Service_delete_instanceInbound(exchangeInfo, null);

			// Scenario.
			Controller.Scenario = new ScenarioPresentation(this, ScenarioType.Service);
			ArgumentOVController lArgumentObjectValued = null;

			// p_thisRevisarAeronave
			lArgumentObjectValued = Controller.InputFields["p_thisRevisarAeronave"] as ArgumentOVController;
			lArgumentObjectValued.Label = new LabelPresentation(this.lp_thisRevisarAeronave);
			lArgumentObjectValued.Editors[0] = new MaskedTextBoxPresentation(this.maskedTextBoxp_thisRevisarAeronaveid_RevisarAeronave1);
			lArgumentObjectValued.Trigger = new ButtonPresentation(this.bp_thisRevisarAeronave);
			if (lArgumentObjectValued.SupplementaryInfo != null)
			{
				lArgumentObjectValued.SupplementaryInfo.Viewer = new LabelDisplaySetPresentation(this.lp_thisRevisarAeronaveSupInfo);
			}
