...
namespace SIGEM.Client.InteractionToolkit.PasajeroAeronave.IUServices
{
	...
	public partial class SIU_delete_instanceInboundIT : Form
	{
		...
		public SIU_delete_instanceInboundIT()
		{
			...
			// Icon assignament.
			this.Icon = UtilFunctions.BitmapToIcon(Properties.Resources.PasajeroAeronave_SIU_SIU_delete_instance);
		}
		...
		{
			Controller = ControllerFactory.PasajeroAeronave.Service_delete_instanceInbound(exchangeInfo, null);

			// Scenario.
			Controller.Scenario = new ScenarioPresentation(this, ScenarioType.Service);
			ArgumentOVController lArgumentObjectValued = null;

			// p_thisPasajeroAeronave
			lArgumentObjectValued = Controller.InputFields["p_thisPasajeroAeronave"] as ArgumentOVController;
			lArgumentObjectValued.Label = new LabelPresentation(this.lp_thisPasajeroAeronave);
			lArgumentObjectValued.Editors[0] = new MaskedTextBoxPresentation(this.maskedTextBoxp_thisPasajeroAeronaveid_PasajeroAeronave1);
			lArgumentObjectValued.Trigger = new ButtonPresentation(this.bp_thisPasajeroAeronave);
			if (lArgumentObjectValued.SupplementaryInfo != null)
			{
				lArgumentObjectValued.SupplementaryInfo.Viewer = new LabelDisplaySetPresentation(this.lp_thisPasajeroAeronaveSupInfo);
			}
