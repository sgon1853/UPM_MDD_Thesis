...
namespace SIGEM.Business.XML
{
	...
	/// </summary>
	[ONGateXMLAttribute("NaveNodriza")]
	internal class NaveNodrizaXml : ONXml
	{
		#region Properties
		...
		/// Default Constructor
		/// </summary>
		public  NaveNodrizaXml() : base("NaveNodriza")
		{
		}
		#endregion
		...
		{
			// Process the service arguments
			ONServiceInfo lSInfo = new ONServiceInfo("Clas_1347649273856884Ser_2_Alias", NaveNodrizaClassText.Delete_instanceServiceAlias, "Clas_1347649273856884_Alias", NaveNodrizaClassText.ClassAlias);
			lSInfo.AddOIDArgument("p_thisNaveNodriza", false, "NaveNodriza", "Clas_1347649273856884Ser_2Arg_1_Alias", NaveNodrizaClassText.Delete_instance_P_thisNaveNodrizaArgumentAlias);
			...
			{
				throw new ONServiceArgumentsException(e, "Clas_1347649273856884_Alias", NaveNodrizaClassText.ClassAlias, "Clas_1347649273856884Ser_2_Alias", NaveNodrizaClassText.Delete_instanceServiceAlias);
			}
			...
			try
			{
				...
				{	
					lServer.Delete_instanceServ(lP_thisNaveNodrizaArg);
					lInstance = lServer.Instance;
				}	
				ticket = lOnContext.GetTicket(dtdVersion, clientName);
			}
		...
		public override ONCollection QueryByOid(ref string ticket, ref ONOid agentOid, XmlReader xmlReader, ONDisplaySet displaySet, double dtdVersion, string clientName)
		{
			...
			// Execute
			NaveNodrizaQuery lQuery = new NaveNodrizaQuery(lOnContext);
		...
		/// <param name="xmlReader">XML with the request message</param>
		/// <param name="dtdVersion">Version of DTD that follows the XML message</param>
		public NaveNodrizaLMDFilter GetFilter_LMD(XmlReader xmlReader, double dtdVersion)
		{
			// Create filter
			NaveNodrizaLMDFilter lFilter = new NaveNodrizaLMDFilter();
			
			// Process the variable filters
			ONServiceInfo lSInfo = new ONServiceInfo("", "LMD", "Clas_1347649273856884_Alias", "NaveNodriza");
			...
			{
				throw new ONServiceArgumentsException(e, "Clas_1347649273856884_Alias", "NaveNodriza", "", "LMD");
			}
		...
		/// <param name="xmlReader">XML with the request message</param>
		/// <param name="dtdVersion">Version of DTD that follows the XML message</param>
		public NaveNodrizaLMDFilter GetFilter_FUM(XmlReader xmlReader, double dtdVersion)
		{
			// Create filter
			NaveNodrizaLMDFilter lFilter = new NaveNodrizaLMDFilter();
			
			// Process the variable filters
			ONServiceInfo lSInfo = new ONServiceInfo("", "FUM", "Clas_1347649273856884_Alias", "NaveNodriza");
			...
			{
				throw new ONServiceArgumentsException(e, "Clas_1347649273856884_Alias", "NaveNodriza", "", "FUM");
			}
		...
		public override void ONQuery2XML(ONOid agentOid, XmlWriter xmlWriter, ONCollection val, ONOid startRowOID, int blockSize, ONDisplaySet displaySet, double dtdVersion)
		{
			...
			xmlWriter.WriteAttributeString("Class", "NaveNodriza");
			xmlWriter.WriteStartElement("Head.OID.Field");
			xmlWriter.WriteAttributeString("Name", "id_NaveNodriza");
			...
			foreach (ONDisplaySetItem lDisplaySetItem in displaySet)
			{
				...
				string lType = ONInstance.GetTypeOfAttribute(typeof(NaveNodrizaInstance), new ONPath(lDisplaySetItem.Path));
				if(lType == "")
					lType = "string";
			...
			{
				NaveNodrizaInstance lInstance = val[i++] as NaveNodrizaInstance;
			
				xmlWriter.WriteStartElement("R");
				ONXmlAutonumeric.ON2XML(xmlWriter, lInstance.Oid.Id_NaveNodrizaAttr, dtdVersion, ONXml.XMLTAG_O);
