...
namespace SIGEM.Business.SQL
{
	...
	/// ONSqlForAllDistinct
	/// </summary>
	public class ONSqlForAllDistinct : ONSqlScalar
	{
		#region Members
		/// <summary>
		...
		#region Select
		/// <summary>
		/// Adds to the SELECT part of the SQL sentence the reserved word to do the required action for FOR All Distinct
		/// </summary>
		public void AddSelect(string alias, string field)
		{
			...
			if (mDistinct.Count == 0)
			{
				...
				lTextBuilder.Append("COUNT(*) - COUNT(DISTINCT ");
		...
		public override string GenerateSQL(out ArrayList sqlParameters)
		{
			...
			{
				lSql.Append("SELECT COUNT(*) - COUNT(DISTINCT ");
				lSql.Append("lAux.");
				lSql.Append(mField);
				lSql.Append(") FROM (");
				lSql.Append(base.GenerateSQL(out sqlParameters));
