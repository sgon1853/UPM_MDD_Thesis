...
{
	[ONDataClass("NaveNodriza")]
	internal class NaveNodrizaData : ONDBData
	{
		#region Constants
		...
		{
			NaveNodrizaCollection lQuery = null;
			...
			try
			{
				...
				NaveNodrizaInstance lAntInstance = null;
				if (lDataReader != null)
				{
					object[] lColumns;
					if(displaySet == null)
					...
					lQuery = new NaveNodrizaCollection(OnContext);
			...
			{
				string ltraceItem = "Method: ExecuteSql, Component: NaveNodrizaData";
		...
		#region Autonumeric
		/// <summary>Obtains the last autonumeric that has the attribute id_NaveNodriza</summary>
		public ONInt GetAutonumericid_NaveNodriza()
		{
			ONSqlMax lOnSql = new ONSqlMax();
			string lAlias = lOnSql.CreateAlias(CtesBD.TBL_NAVENODRIZA, null, "NaveNodriza");
			lOnSql.AddSelect(lAlias, CtesBD.FLD_NAVENODRIZA_ID_NAVENODRIZA);
		...
		#region Facet 'NaveNodriza'
		private static string NaveNodrizaFacetAddSql(ONSqlSelect onSql, ONPath onPath)
		{
			return NaveNodrizaFacetAddSql(JoinType.InnerJoin, onSql, onPath, false, false);
		}
		private static string NaveNodrizaFacetAddSql(ONSqlSelect onSql, ONPath onPath, bool force)
		{
			return NaveNodrizaFacetAddSql(JoinType.InnerJoin, onSql, onPath, force, false);
		}
		private static string NaveNodrizaFacetAddSql(JoinType joinType, ONSqlSelect onSql, ONPath onPath, bool force)
		{
			return NaveNodrizaFacetAddSql(joinType, onSql, onPath, force, false);
		}
		...
		public static string AddPath(ONSqlSelect onSql, JoinType joinType, string facet, ONPath onPath, ONPath processedOnPath, string initialClass, bool forceLastAlias, bool isLinkedTo)
		{
			...
			{
				if (forceLastAlias)
					return NaveNodrizaFacetAddSql(joinType, onSql, processedOnPath, forceLastAlias, isLinkedTo);

				if ((processedOnPath == null) || (processedOnPath.Count == 0))
					return (onSql.CreateAlias(joinType, "", CtesBD.TBL_NAVENODRIZA, null, "NaveNodriza", false, isLinkedTo));
				else
					return (onSql.CreateAlias(joinType, "", CtesBD.TBL_NAVENODRIZA, processedOnPath, "NaveNodriza", false, isLinkedTo));
			}
		...
		{
			FixInstance(onSql, onPath, OnPath, oid as NaveNodrizaOid);
		}

		public override void InhFixInstance(ONSqlSelect onSql, ONPath onPath, ONPath OnPath, ONOid oid, bool isLinkedTo)
		{
			FixInstance(onSql, onPath, OnPath, oid as NaveNodrizaOid, isLinkedTo);
		}
		...
		public override int CompareUnionOID(ONInstance instance1, ONInstance instance2)
		{
			...
			NaveNodrizaInstance lInstance1 = instance1 as NaveNodrizaInstance;
			NaveNodrizaInstance lInstance2 = instance2 as NaveNodrizaInstance;

			int lCompare = 0;

			// id_NaveNodriza(Asc)
			lCompare = lInstance1.Id_NaveNodrizaAttr.CompareTo(lInstance2.Id_NaveNodrizaAttr);
			if (lCompare != 0)
				return lCompare;
