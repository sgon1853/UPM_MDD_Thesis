...
{
	[ONDataClass("Aeronave")]
	internal class AeronaveData : ONDBData
	{
		#region Constants
		...
		#region Autonumeric
		/// <summary>Obtains the last autonumeric that has the attribute id_Aeronave</summary>
		public ONInt GetAutonumericid_Aeronave()
		{
			ONSqlMax lOnSql = new ONSqlMax();
			string lAlias = lOnSql.CreateAlias(CtesBD.TBL_AERONAVE, null, "Aeronave");
			lOnSql.AddSelect(lAlias, CtesBD.FLD_AERONAVE_ID_AERONAVE);
		...
		public void PasajeroAeronaveRoleDelete(AeronaveOid localOid, PasajeroAeronaveOid relatedOid)
		{
			ONSqlUpdate lOnSql = new ONSqlUpdate();
			lOnSql.AddUpdate(CtesBD.TBL_PASAJEROAERONAVE);
			lOnSql.AddSet(CtesBD.FLD_PASAJEROAERONAVE_FK_AERONAVE_1, ONInt.Null);
			lOnSql.AddWhere(CtesBD.FLD_PASAJEROAERONAVE_FK_AERONAVE_1, localOid.Id_AeronaveAttr);
			lOnSql.AddWhere(CtesBD.FLD_PASAJEROAERONAVE_ID_PASAJEROAERONAVE, relatedOid.Id_PasajeroAeronaveAttr);
			Execute(lOnSql);
		}
		...
		private static string AeronaveFacetAddSql(ONSqlSelect onSql, ONPath onPath, bool force)
		{
			return AeronaveFacetAddSql(JoinType.InnerJoin, onSql, onPath, force, false);
		}
		...
		public static ONText LoadTextDestino(ONContext onContext, AeronaveOid oid)
		{
			...
			lOnSql.CreateAlias(CtesBD.TBL_AERONAVE, null, "Aeronave");
			lOnSql.AddSelect(CtesBD.FLD_AERONAVE_DESTINO);
